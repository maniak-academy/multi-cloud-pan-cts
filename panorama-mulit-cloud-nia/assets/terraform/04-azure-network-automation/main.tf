...
resource "azurerm_network_interface" "ctsnic" {
  ...
  ip_configuration {
    ...
    subnet_id                     = data.terraform_remote_state.environment.outputs.app_network_mgmt_subnet
...
resource "azurerm_linux_virtual_machine" "consul-terraform-sync" {
  ...
  custom_data          = base64encode(templatefile("./scripts/consul-tf-sync.sh", { 
    ...
    panos_mgmt_addr = data.terraform_remote_state.fw.outputs.pan-azure-mgmt-public,
