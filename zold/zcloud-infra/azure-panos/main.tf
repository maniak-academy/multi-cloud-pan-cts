...
resource "azurerm_storage_account" "pan_fw_stg_ac" {
  name                     = "strgaccpafw${random_id.pansuffix.dec}"
...
resource "azurerm_public_ip" "PublicIP_0" {
  name                = "fwMgmtPublicIP-${random_id.pansuffix.dec}"
  location            = var.location
  resource_group_name = var.resource_group_name
  allocation_method   = "Static"
  domain_name_label   = "${var.FirewallDnsName}-${random_id.pansuffix.dec}"
}

resource "azurerm_public_ip" "PublicIP_1" {
  name                = "webPublicIP-${random_id.pansuffix.dec}"
  location            = var.location
  resource_group_name = var.resource_group_name
  allocation_method   = "Static"
  domain_name_label   = "${var.WebServerDnsName}-${random_id.pansuffix.dec}"
}

resource "azurerm_network_interface" "VNIC0" {
  name                = "FWeth0-${random_id.pansuffix.dec}"
  location            = var.location
  resource_group_name = var.resource_group_name
...
resource "azurerm_network_interface" "VNIC1" {
  name                = "FWeth1-${random_id.pansuffix.dec}"
  location            = var.location
  resource_group_name = var.resource_group_name
...
resource "azurerm_network_interface" "VNIC2" {
  name                = "FWeth2-${random_id.pansuffix.dec}"
  location            = var.location
  resource_group_name = var.resource_group_name
...
resource "azurerm_virtual_machine" "PAN_FW_FW" {
  name                = "vmPanOS-${random_id.pansuffix.dec}"
  location            = var.location
  resource_group_name = var.resource_group_name
  ...
  depends_on = [azurerm_network_interface.VNIC0,
    ...
    azurerm_storage_account.pnfwbootstrap
  ]
  plan {
    name      = var.fwSku
  ...
  storage_os_disk {
    name          = "vmPANW-${random_id.pansuffix.dec}-osDisk"
    vhd_uri       = "${azurerm_storage_account.pan_fw_stg_ac.primary_blob_endpoint}vhds/vmPANW-${random_id.pansuffix.dec}-${var.fwOffer}-${var.fwSku}.vhd"
    caching       = "ReadWrite"
    create_option = "FromImage"
  }

  os_profile {
    computer_name  = "vmPANW-${random_id.pansuffix.dec}"
    admin_username = var.adminUsername
    admin_password = random_password.pafwpassword.result
    custom_data = join(
    ",",
    [
     "storage-account=${azurerm_storage_account.pnfwbootstrap.name}",
     "file-share=${azurerm_storage_share.pnfwshare.name}",
