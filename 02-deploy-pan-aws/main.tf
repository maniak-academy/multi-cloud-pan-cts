...
resource "random_id" "pansuffixaws" {
  byte_length = 3
}

resource "random_integer" "password-lengthaws" {
  min = 12
  max = 25
}

resource "random_password" "pafwpasswordaws" {
  length           = random_integer.password-lengthaws.result
  min_upper        = 1
  min_lower        = 1
  min_numeric      = 1
  min_special      = 1
...
module "vmseries" {
  ...
  bootstrap_options = join(";",
    [
      "type=dhcp-client",
      "hostname=${var.me}${random_id.pansuffixaws.dec}",
